...
### Banking wire (`shop-bridge`)

1. KafkaUser `npd-banking` Ready  
2. Copy secrets:

```bash
# từ repo banking-demo
bash phase9-gitops-platform/environments/dev-ocp/scripts/sync-kafka-client-secrets.sh
```

3. Build/push `shop-bridge` → sync Argo `banking-shop-bridge`  
4. Lab: auto-confirm bật → đặt hàng shop → `payments.events` → đơn paid  
5. CK tay: `SHOP_BRIDGE_AUTO_CONFIRM=false` + `POST /api/shop/confirm-payment`

Chi tiết: `banking-demo/.../kafka/DEPLOY.md` §4b.
